...
export const normalizeWord = (input: string): string => input.trim().toLowerCase();
...
export const drawWeightedLetter = (kind: PickKind, random: () => number): string => {
  const weights = kind === "vowel" ? vowelWeights : consonantWeights;
